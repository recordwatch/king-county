...
export async function runScrape({
  ...
  fetchReleaseTimes,
  // SCORE only: re-checks releases still marked 'detected' against that
  // person's own Booking List, which eventually publishes a real Date
  // Released even when /recentreleases couldn't (see scrapers/score.js
  // recheckDetectedReleases). Optional; unset for every other source.
  recheckDetected,
  recheckBatch = 100,
  // Give up re-checking a 'detected' release once it's this old and leave it
  // as detected permanently, rather than re-fetching it forever if the
  // source's own history page never ends up publishing a matching row.
  recheckMaxAgeDays = 14,
}) {
  ...
  let records;
  try {
    records = await fetchRoster({ roster });
  } catch (err) {
  ...
  // For explicitStatus sources (KC DAJD's rolling Socrata window), a stored
  // in_custody entry can legitimately not appear in `records` at all -- its
  // booking aged out of the window and wasn't resolved by a targeted
  // by-number check either -- and is deliberately left untouched rather than
  // released. Count those toward the "new" side too, so this comparison
  // measures the same population on both sides instead of penalizing people
  // we're intentionally not touching this run. Non-explicit sources don't
  // need this: their fetch already IS the complete current live roster, and
  // anyone missing from it is genuinely gone -- exactly what the
  // disappearance-based release logic further down is supposed to catch,
  // not something to paper over here.
  const recordIds = new Set(records.map(r => r.idnum));
  const untouchedInCustody = explicitStatus
    ? Object.values(roster).filter(e => e.status === 'in_custody' && !recordIds.has(e.idnum)).length
    : 0;
  const newInCustody = records.filter(r => (r.status || 'in_custody') === 'in_custody').length + untouchedInCustody;
  ...
  for (const { r, key, oldKey } of freshEntries) {
    ...
    const entry = {
      ...
      // Booking-level release-status text a source's roster page gives
      // alongside bail (currently just Kent, e.g. "Bail Denied", "Sentenced
      // - Release Date ...") -- null for everyone else.
      ...
      // The booking's aggregate bail total, when a source's detail fetch
      // provides it (currently just Kent) -- null for everyone else.
      totalBail: detail && detail.totalBail ? detail.totalBail : null,
      // A person's full booking history at the source, when a source's
      // detail fetch provides it (currently just SCORE's Booking List,
      // which -- unlike Kent's priorBookings -- includes the current
      // booking's own row too) -- null for everyone else.
      bookingHistory: detail && detail.bookingHistory ? detail.bookingHistory : null,
      // Set only when a 'detected' release is later upgraded to 'county' via
      // recheckDetected -- preserves the original scrape-detection guess
      // rather than overwriting it. Null until/unless that happens.
  ...
  for (const { r, key } of existingRecords) {
    ...
    existing.releaseType = r.releaseType || null;
    ...
    if (logEntry) {
      ...
      logEntry.releaseType = existing.releaseType;
    ...
    if (needsDetail.length > 0) {
      ...
      try {
        ...
        for (const rawId of needsDetail) {
          ...
          const totalBail = detail.totalBail || null;
          const bookingHistory = detail.bookingHistory || roster[key].bookingHistory || null;
          roster[key] = { ...roster[key], charges: detail.charges, priorBookings, totalBail, bookingHistory, hasDetail: !!detail.complete };
          const logEntry = log.find(e => e.idnum === key);
          if (logEntry) Object.assign(logEntry, { charges: detail.charges, priorBookings, totalBail, bookingHistory, hasDetail: !!detail.complete });
        }
      } catch (err) {
  ...
  if (explicitStatus) {
    ...
    function applyRelease(key, logLabel) {
      ...
      // scheduledReleaseDate is deliberately left as whatever was last seen
      // while still in custody (Kirkland's projected/scheduled date column)
      // rather than wiped -- it's never used as a release time anywhere
      // (only shown informationally, and only while still in_custody), so
      // keeping it costs nothing and preserves a real data point.
      const logEntry = log.find(e => e.idnum === key);
      if (logEntry) {
  ...
  // Re-check releases still stuck on our own scrape-detection guess against
  // the source's fuller history (currently just SCORE's Booking List) --
  // upgrades to 'county' once that source publishes a real release time for
  // the specific booking. Capped per run (recheckBatch) since each check is
  // its own request; recheckDetected itself spaces those requests out. Also
  // capped by age (recheckMaxAgeDays) -- if a source never ends up
  // publishing a matching row, there's no point re-fetching it forever; past
  // that age it's left as 'detected' for good.
  if (recheckDetected) {
    // For a 'detected' release, releasedAt IS the original scrape-detection
    // guess (resolveRelease only sets releaseSource: 'detected' when it fell
    // back to `now`) -- the right anchor for "how long have we been trying."
    const maxAgeMs = recheckMaxAgeDays * 24 * 60 * 60 * 1000;
    const pending = Object.values(roster)
      .filter(e => e.source === sourceId && e.status === 'released' && e.releaseSource === 'detected')
      .filter(e => {
        const detectedAt = new Date(e.releasedAt).getTime();
        return isNaN(detectedAt) || Date.now() - detectedAt <= maxAgeMs;
      })
      .slice(0, recheckBatch);
    if (pending.length > 0) {
      console.log(`  Re-checking ${pending.length} detected release(s) against booking history...`);
      let updates = new Map();
      try {
        updates = await recheckDetected(pending.map(e => ({ idnum: e.idnum, bookingNumber: e.bookingNumber })));
      } catch (err) {
        console.warn('  Detected-release recheck failed:', err.message);
      }
      let upgraded = 0;
      for (const [key, update] of updates) {
        const existing = roster[key];
        if (!existing) continue;
        if (update.bookingHistory) existing.bookingHistory = update.bookingHistory;
        if (isValidDate(update.releasedAt)) {
          existing.detectedReleasedAt = existing.detectedReleasedAt ?? existing.releasedAt;
          existing.releasedAt = update.releasedAt;
          existing.releaseReason = update.releaseReason || existing.releaseReason;
          existing.releaseSource = 'county';
          upgraded++;
        }
        const logEntry = log.find(e => e.idnum === key);
        if (logEntry) {
          logEntry.bookingHistory = existing.bookingHistory;
          logEntry.detectedReleasedAt = existing.detectedReleasedAt;
          logEntry.releasedAt = existing.releasedAt;
          logEntry.releaseReason = existing.releaseReason;
          logEntry.releaseSource = existing.releaseSource;
        }
      }
      if (upgraded > 0) console.log(`  Upgraded ${upgraded} detected release(s) to county via booking history.`);
    }
  }
