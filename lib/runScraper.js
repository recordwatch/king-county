...
export async function runScrape({
  ...
  if (explicitStatus) {
    ...
    for (const id of releasedIds) {
      ...
      const infoReleasedAt = (typeof info === 'string' ? info : info?.releasedAt) || null;
      const releasedAt = infoReleasedAt || now;
      const releaseReason = (typeof info === 'object' ? info?.releaseReason : null) || null;
      // "county" only when fetchReleaseTimes returned a non-empty timestamp
      // for this id AND it actually parses as a valid date -- not just
      // whether `info` itself is truthy, since an object with a blank,
      // missing, or garbage releasedAt still falls back to `now` above and
      // must be "detected", not "county". "detected" also covers every Kent
      // and Kirkland release, since neither supplies fetchReleaseTimes at all.
