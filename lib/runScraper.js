...
export async function runScrape({
  ...
  const drop = oldInCustody > 0 ? (oldInCustody - newInCustody) / oldInCustody : 0;
  if (oldInCustody > 0 && drop > 0.10 && process.env.FORCE_UPDATE !== 'true') {
    console.error(`  ABORTED: in-custody count dropped ${(drop * 100).toFixed(1)}% (${oldInCustody} -> ${newInCustody}), more than the 10% safety threshold. Set FORCE_UPDATE=true to override.`);
    process.exitCode = 1;
    return;
  }
