...
export async function runScrape({
  ...
  if (fetchDetailBatch && newRecords.length > 0 && newRecords.length <= detailBatchLimit) {
    ...
    try {
      detailMap = await fetchDetailBatch(newRecords.map(r => r.idnum));
    } catch (err) {
  ...
  for (const r of newRecords) {
    ...
    const entry = {
      ...
      hasDetail: fetchDetailBatch ? !!detail : true,
    };
  ...
  // Backfill details for anyone still missing them (first-run overflow, or
  // previous detail fetch failures).
  if (fetchDetailBatch) {
    ...
    if (needsDetail.length > 0) {
      ...
      try {
        const backfillMap = await fetchDetailBatch(needsDetail);
        for (const id of needsDetail) {
          const detail = backfillMap[id];
          if (!detail) continue;
          roster[id] = { ...roster[id], charges: detail.charges, hasDetail: true };
