...
**Decided against (2026-09-15):** cross-referencing long releases against WA DOC's inmate search to detect jail-to-prison transfers. Investigated and found the "WA DOC inmate search" is actually a white-labeled instance of VINE (vinelink.vineapps.com, the national Appriss victim-notification platform -- the same VINE linked from SCORE's roster). It's a fully client-side SPA with no static HTML fallback (would need real headless-browser automation, not a simple HTTP scrape) and is a single-person-lookup tool, meaning bulk cross-referencing would mean hundreds of individual automated searches against a system whose stated purpose is crime-victim safety notification. User chose to skip this rather than pursue it.
